...
function factorsOf(n) {
    if(Number.isNaN(Number(n))) {
        throw new RangeError("Argument Error: Value must be an interger");
    }
    if(n < 0) {
        throw new RangeError("Argument Error: Number must be an positive");
    }
    if (!Number.isInteger(n)) {
      throw new RangeError("Argument Error: Number must be an interger");
    }
    const factors = [];
...
test("non-interger numbers return not prime", () => {
